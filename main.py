...
async def restart_handler(_, m):
    global STOP_PROCESS
    STOP_PROCESS = True
    await m.reply_text("🛑 **Stopping... The bot will generate the index for uploaded files and then stop.**", True)
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:    
        ...
        links = []
        for i in content:
            # ✅ New Topic-Wise Format Parsing
            if " || " in i and " : " in i:
                try:
                    # Split Metadata and URL (Right to Left)
                    meta_part, url_part = i.rsplit(" : ", 1)
                    
                    # Clean URL (remove protocol for compatibility)
                    url_clean = url_part.split("://", 1)[1] if "://" in url_part else url_part

                    # Split Metadata: Title || Topic || Subject
                    meta_parts = meta_part.rsplit(" || ", 2)
                    
                    if len(meta_parts) == 3:
                        title_raw = meta_parts[0].strip()
                        topic = meta_parts[1].strip()
                        subject = meta_parts[2].strip()
                        
                        # 🧹 Clean Title: Remove Topic if present
                        if topic in title_raw:
                            title_raw = title_raw.replace(topic, "").strip()
                            title_raw = title_raw.replace("- -", "-").replace("  ", " ") # Fix double separators
                            if title_raw.endswith("-"): title_raw = title_raw[:-1].strip()
                        
                        links.append([title_raw, url_clean, subject])
                        continue
                except Exception:
                    pass # Fallback to standard parsing if format fails
            ...
            if "://" in i:
                ...
                if len(parts) == 2:
                    name = parts[0]
                    url = parts[1]
                    links.append([name, url, None]) # None for subject
    ...
    chat_id = editable.chat.id
    timeout_duration = 3 if auto_flags.get(chat_id) else 20
    await editable.edit("__**📢 Provide the Topic ID or send /d__\n\n<blockquote>🔹Send Your Topic ID if using a Forum Channel.\n\nEx : 123</blockquote>\n**")
    try:
        input8: Message = await bot.listen(editable.chat.id, timeout=timeout_duration)
    ...
    try:
        if raw_text == "1":
            ...
            batch_message = await bot.send_message(chat_id=channel_id, text=f"<blockquote><b>🎯Target Batch : {b_name}</b></blockquote>", message_thread_id=topic_id)
    ...
    current_subject = None # 🧠 Track current subject for headers
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            # 📢 Send Subject Header if changed
            topic = None
            if subject and subject != current_subject:
                current_subject = subject
                subj_msg = await bot.send_message(channel_id, f"**{current_subject}**", message_thread_id=topic_id)
                if subject not in index_mapping:
                    index_mapping[subject] = {'link': subj_msg.link, 'topics': {}}
            
            # Extract topic from raw name if available (based on parsing logic)
            # Re-parsing strictly for topic extraction if needed, or relying on previous parsing
            # Since 'links' only stores [title, url, subject], we need to re-extract topic if we want to group by it.
            # However, the previous parsing logic in 'links' construction didn't store 'topic' explicitly in the list.
            # Let's assume we can re-parse 'i' from content or just use the logic below if we had stored it.
            # To fix this properly without changing 'links' structure too much, we can re-parse 'name_raw' or the original line if available.
            # But wait, in the previous step we cleaned 'name_raw'. 
            # Let's rely on the fact that we need to capture the topic for the index.
            
            # Quick fix: We need 'topic' for the index. 
            # Let's look at how 'links' was populated. 
            # It was: links.append([title_raw, url_clean, subject])
            # We lost 'topic' in the list. 
            # We should probably update the parsing loop to include topic, but for now let's try to infer it or just proceed.
            # Actually, let's look at the parsing loop again.
            # You requested to use the 'topic' from the parsing. 
            # I will assume 'links' has 4 elements now: [title, url, subject, topic] or I will re-parse here.
            # Since I cannot change the parsing loop in this diff block easily without context, 
            # I will assume we can get topic from the original content list if we had index 'i'.
            # But 'links' is a separate list.
            
            # Let's just use a simple re-parse of the original line logic here for the index feature to work perfectly.
            # Or better, let's assume the user accepts that I modify the parsing loop above to include topic.
            
            # For this specific request, I will implement the Index Generation logic assuming 'topic' is available.
            # I will add a small re-parse logic here for safety.
            topic = None
            if " || " in content[i] and " : " in content[i]:
                 try:
                    meta_part = content[i].rsplit(" : ", 1)[0]
                    meta_parts = meta_part.rsplit(" || ", 2)
                    if len(meta_parts) == 3:
                        topic = meta_parts[1].strip()
                 except: pass

            Vxy = url_raw.replace("file/d/","uc?export=download&id=").replace("www.youtube-nocookie.com/embed", "youtu.be").replace("?modestbranding=1", "").replace("/view?usp=sharing","")
            url = "https://" + Vxy
            link0 = "https://" + Vxy

            name1 = name_raw.replace("(", "[").replace(")", "]").replace("_", "").replace("\t", "").replace(":", "").replace("/", "").replace("+", "").replace("#", "").replace("|", "").replace("@", "").replace("*", "").replace(".", "").replace("https", "").replace("http", "").strip()
            if "," in raw_text3:
    ...
    f"\n\n<b>🎓  Uᴘʟᴏᴀᴅ Bʏ : {CR}</b>"
                ...
                if "drive" in url:
                    try:
                        ...
                        copy = await bot.send_document(chat_id=channel_id,document=ka, caption=cc1, message_thread_id=topic_id)
                        count+=1
                        os.remove(ka)
                    ...
                    try:
                        ...
                        copy = await bot.send_document(chat_id=channel_id, document=f"{name}.html", caption=cchtml, message_thread_id=topic_id)
                    ...
                    try:
                        ...
                        copy = await bot.send_photo(chat_id=channel_id, photo=f'{name}.{ext}', caption=ccimg, message_thread_id=topic_id)
                        count += 1
                    ...
                    try:
                        ...
                        copy = await bot.send_document(chat_id=channel_id, document=f'{name}.{ext}', caption=cc1, message_thread_id=topic_id)
                        os.remove(f'{name}.{ext}')
                    except FloodWait as e:
                        await m.reply_text(str(e))
                        time.sleep(e.x)
                    ...
                    try:
                        ...
                        if os.path.exists(filename):
                            ...
                            copy = await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark, topic_thread_id=topic_id)
                            count += 1
                        else:
                            await bot.send_message(channel_id, f'⚠️**Downloading Failed**⚠️\n**Name** =>> `{str(count).zfill(3)} {name1}`\n**Url** =>> {link0}\n\n<blockquote><i><b>Failed Reason: {str(e)}</b></i></blockquote>', disable_web_page_preview=True)
                            await bot.send_message(channel_id, f'⚠️**Downloading Failed**⚠️\n**Name** =>> `{str(count).zfill(3)} {name1}`\n**Url** =>> {link0}\n\n<blockquote><i><b>Failed Reason: {str(e)}</b></i></blockquote>', disable_web_page_preview=True, message_thread_id=topic_id)
                            failed_count += 1
                            count += 1
                            continue
                ...
                # 📝 Index Logic: Store Topic Link
                if copy and subject and topic:
                    if subject in index_mapping:
                        if topic not in index_mapping[subject]['topics']:
                            index_mapping[subject]['topics'][topic] = copy.link
    ...
    # 📊 Generate and Send Index
    if index_mapping:
        index_text = f"<u><b>{b_name}</b></u>\n\n"
        for subj, data in index_mapping.items():
            subj_link = data.get('link')
            if subj_link:
                index_text += f"<a href='{subj_link}'><b>{subj}</b></a>\n"
            else:
                index_text += f"<b>{subj}</b>\n"
            
            for top, link in data['topics'].items():
                index_text += f"   |- <a href='{link}'>{top}</a>\n"
            index_text += "\n"
        
        try:
            await bot.send_message(channel_id, index_text, disable_web_page_preview=True, message_thread_id=topic_id)
        except Exception as e:
            await m.reply_text(f"⚠️ Failed to send Index: {e}")
